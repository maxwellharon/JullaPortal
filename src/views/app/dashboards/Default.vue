...
<div >
    ...
    <b-row>
        ...
        <b-colxx xl="6" lg="12">
            <div class="icon-cards-row" v-for="(value, key , user) in post.data.slice(0,1) " :key="key" :user="user">
              <div id="app">
                <p v-if="loading">Loading...</p>
                <ul v-else>
                <glide-component :settings="glideIconsOption">
                    <icon-card :title="$t('dashboards.pending-requests')" icon="simple-icon-clock" :value="800" />
                    <icon-card :title="$t('dashboards.accepted-requests')" icon="simple-icon-check" :value="300" />
                    <icon-card :title="$t('dashboards.total-requests')" icon="simple-icon-list" :value="1100" />
                </glide-component>
              </ul>
            </div>
          </div>
                ...
                <b-colxx md="12" class="mb-4">
                    <b-card :title="$t('dashboards.sales')">
                        ...
                        <div class="dashboard-line-chart">
                            <line-shadow-chart :data="lineChartData" :height="250" />
                        </div>
                    </b-card>
                </b-colxx>
            </b-row>
        ...
        <b-colxx lg="12" xl="6" class="mb-4">
            <b-card :title="$t('dashboards.recent-requests')">
                <b-refresh-button @click="refreshButtonClick" />
                <vue-perfect-scrollbar class="scroll dashboard-list-with-thumbs" :settings="{ suppressScrollX: true, wheelPropagation: false }">
                    <recent-order-item v-for="(order,index) in products.slice(0,1)" :order="order" detail-path="/app/pages/product/details" :key="index" />
                </vue-perfect-scrollbar>
            </b-card>
            <!-- <div id="app">
            <p v-if="loading">Loading...</p>
            <ul v-else>
              <li v-for="(value, key , user) in post.data" :key="key" :user="user">
                 {{ value.user.name }}
              </li>
            </ul>
          </div> -->
    ...
    <!-- <draggable class="row">
        ...
        <b-colxx xl="3" lg="6" class="mb-4">
            <radial-progress-card :title="$t('dashboards.paymenrecent-requeststs-done')" :percent="45" />
        </b-colxx>
    </draggable> -->
